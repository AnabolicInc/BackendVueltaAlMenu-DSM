...
const payment = async (req = request, res = response) => {
	try {
		const { holder_name, card_number, expiration_date, cvc, session_token, cuotes, type, user_id, order_id } = req.body;
		const payment = new Payment({ holder_name, card_number, expiration_date, cvc, session_token, cuotes, type, user_id, order_id });
		await payment.save();
		res.status(201).json({
			success: true,
			data: payment,
